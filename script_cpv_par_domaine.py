...
def script_cpv_maxAwardPrice_Buyers(conn, cpv_max_nb, top_max_nb):
    ...
    plt.tight_layout()
    generateFileChart("Fusion", f"Top {top_max_nb} CPV pour les acheteurs qui génèrent le plus d'argent",
                      "hist_with_log")
...
def script_cpv_maxAwardPrice_Suppliers(conn, cpv_max_nb, top_max_nb):
    ...
    plt.yscale("log")
    plt.title(f'Top {top_max_nb} CPV pour les fournisseurs qui gagnent le plus d\'argent')
    plt.xticks(rotation=45)
...
def script_cpv_flux(conn, cpv_max_nb, top_max_nb):
    ...
    merged_df['combined_count'] = merged_df['buyers_count'] + merged_df['suppliers_count']
...
def script_cpv_domain(conn, typeContract):
    ...
    top_cpvs_query = create_df_from_query(
        conn,
        "SELECT SUBSTR(L.cpv, 1, 2) as cpv_group, SUM(L.awardPrice) as total_award_price, department FROM Lots L JOIN LotSuppliers LS ON L.lotId = LS.lotId JOIN Agents A ON LS.agentId = A.agentId WHERE typeOfContract='"+typeContract+"' AND A.department IN ({}) GROUP BY cpv_group ORDER BY total_award_price DESC LIMIT 10".format(
            ','.join(f"'{d}'" for d in df_top_departments['department']))
    )
    pivot_df = top_cpvs_query.pivot(index='department', columns='cpv_group', values='total_award_price')
    ...
    plt.tight_layout()
    generateFileChart("Fusion", f"Top 10 des CPV TOP 10 les département pour le contrat {typeContract}", "hist")
