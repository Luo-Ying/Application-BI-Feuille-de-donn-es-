...
def get_total(xml):
    ...
    if xml.xpath("name(*) = 'SINGLE_VALUE'"):
        currency, *value = xml.xpath(
            ".//ted:VALUE/@CURRENCY | " ".//ted:VALUE/text()", namespaces=NMSP
        )  # Value Might be missing

        return {"CURRENCY": currency, "VALUE": value}  # Might be missing
    else:
        low_currency, *low_value = xml.xpath(
            ".//ted:VALUE[1]/@CURRENCY | " ".//ted:VALUE[1]/text()", namespaces=NMSP
        )
        high_currency, *high_value = xml.xpath(
            ".//ted:VALUE[2]/@CURRENCY | " ".//ted:VALUE[2]/text()", namespaces=NMSP
        )
...
def get_notice(xml):
    ...
    for item in ["ORIGINAL_NUTS", "ORIGINAL_CPV"]:
        obj[item] = xml.xpath(
            "ted:*[local-name() = $name]/@CODE", namespaces=NMSP, name=item
        )  # 0 or more

    obj["NO_DOC_OJS"] = xml.xpath(
        "ted:NO_DOC_OJS/text()", namespaces=NMSP
    )  # Compulsory, only one

    obj["ISO_COUNTRY"] = xml.xpath(
        "ted:ISO_COUNTRY/@VALUE", namespaces=NMSP
    )  # Compulsory, only one

    obj["IA_URL_GENERAL"] = xml.xpath(
        "ted:IA_URL_GENERAL/text()", namespaces=NMSP
    )  # Optional

    obj["REF_NOTICE"] = xml.xpath(
        "ted:REF_NOTICE/ted:NO_DOC_OJS/text()", namespaces=NMSP
    )  # 0 or more
    ...
    if values:
        ...
        global_val = values[0].xpath(".//ted:VALUES[@TYPE = 'GLOBAL']", namespaces=NMSP)
        if global_val:
            values_list["GLOBAL_VALUE"] = get_total(global_val[0])

        # Extract individual sub-contracts values
        contract_val = values[0].xpath(
            ".//ted:VALUES[@TYPE = 'CONTRACT']", namespaces=NMSP
        )
        if contract_val:
...
def get_codif(xml):
    ...
    for item in [
        ...
        el = xml.xpath("ted:*[local-name() = $name]/@CODE", namespaces=NMSP, name=item)
...
def get_coded(xml):
    ...
    obj["NOTICE_DATA"] = get_notice(
        xml.xpath("ted:CODED_DATA_SECTION/ted:NOTICE_DATA", namespaces=NMSP)[0]
    )
    obj["CODIF_DATA"] = get_codif(
        xml.xpath("ted:CODED_DATA_SECTION/ted:CODIF_DATA", namespaces=NMSP)[0]
    )
...
def get_cost(xml):
    ...
    if xml.xpath("boolean(./ted:RANGE_VALUE_COST)", namespaces=NMSP):
        ...
        obj["LOW_VALUE"] = xml.xpath(
            "./ted:RANGE_VALUE_COST/ted:LOW_VALUE/text()", namespaces=NMSP
        )
        obj["HIGH_VALUE"] = xml.xpath(
            "./ted:RANGE_VALUE_COST/ted:HIGH_VALUE/text()", namespaces=NMSP
        )
...
def get_contract_value(xml):
    ...
    if xml.xpath(
        "boolean(./ted:COSTS_RANGE_AND_CURRENCY_WITH_VAT_RATE)", namespaces=NMSP
    ):
        obj["COST"] = get_cost(
            xml.xpath("./ted:COSTS_RANGE_AND_CURRENCY_WITH_VAT_RATE", namespaces=NMSP)[
                0
            ]
        )

    if xml.xpath(
        "boolean(./ted:INITIAL_ESTIMATED_TOTAL_VALUE_CONTRACT)", namespaces=NMSP
    ):
        obj["ESTIMATE"] = get_cost(
            xml.xpath("./ted:INITIAL_ESTIMATED_TOTAL_VALUE_CONTRACT", namespaces=NMSP)[
                0
            ]
        )
    if xml.xpath(
...
def get_object(xml):
    ...
    obj["NUTS_EXTRA"] = xml.xpath(
        ".//ted:LOCATION/ted:P/text()", namespaces=NMSP
    )  # 0 or more
    obj["CPV_MAIN"] = xml.xpath(
        ".//ted:CPV_MAIN/ted:CPV_CODE/@CODE", namespaces=NMSP
    )  # 0 or 1

    # Step 4.2: Extract CONTRACT_COVERED_GPA, CONCLUSION_FRAMEWORK_AGREEMENT,
    # CONTRACTS_DPS
    obj["CONTRACT_COVERED_GPA"] = xml.xpath(
        ".//ted:CONTRACT_COVERED_GPA/@VALUE", namespaces=NMSP
    )  # 0 or 1

    obj["CONCLUSION_FRAMEWORK_AGREEMENT"] = (
        ["YES"]
        if xml.xpath("boolean(.//ted:CONCLUSION_FRAMEWORK_AGREEMENT)", namespaces=NMSP)
        else ["NO"]
    )  # YES/NO
    ...
    if contact_data:
        ...
        contractor["OFFICIALNAME"] = contact_data.xpath(
            (
                ...
                "./ted:ORGANISATION/text()"
            ),
            namespaces=NMSP,
        )

        # Step 5.1.2: Extract CONTRACTOR ADDRESS
        contractor["COUNTRY"] = contact_data.xpath(
            "ted:COUNTRY/@VALUE", namespaces=NMSP
        )
        contractor["ADDRESS"] = contact_data.xpath(
            "ted:ADDRESS/text()", namespaces=NMSP
        )
        contractor["TOWN"] = contact_data.xpath("ted:TOWN/text()", namespaces=NMSP)
...
def get_contract(xml):
    ...
    contract_object = contract.xpath(  # Compulsory, only one
        "ted:*[starts-with(local-name(), 'OBJECT')]", namespaces=NMSP
    )[0]
    ...
    for award in awards:
        ...
        if award.xpath("boolean(./ted:AWARD_AND_CONTRACT_VALUE)", namespaces=NMSP):

            for sub_award in award.xpath(
                "ted:AWARD_AND_CONTRACT_VALUE", namespaces=NMSP
            ):
