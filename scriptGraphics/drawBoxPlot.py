...
def draw_box_plot_multiple_simple_stats(data, xlabel, ylabel, title, file, log=False, dropNaN=True):
    if dropNaN:
        data = data.dropna(subset=[xlabel, ylabel])
    ...
    selected_indices = [0, len(unique_values) // 2, len(unique_values) - 1]  # exemple: la première, la médiane et la dernière boîte
    stats = pd.DataFrame(index=unique_values[selected_indices], columns=['Median', 'Mean'])
